...
export const register = async (formData: RegisterFormData) => {
    ...
    if (!response.ok) {
        throw new Error(responseBody.message); //message from backend
    }
}
...
export type SearchParams = { // for search params everything is string
    destination?: string; //optional, cuz user can view without searching
    checkIn?: string;
    checkOut?: string;
    adultCount?: string;
    childCount?: string;
...
export const searchHotels = async (searchParams: SearchParams): Promise<HotelSearchResponse> => {
    ...
    searchParams.facilities?.forEach((facility) => { //bcz user can select multiple ones need to do it like this
        queryParams.append("facilities", facility); //similiar to what we did in ManageHotelForm
    });
...
export const fetchHotelById = async (hotelId: string): Promise<HotelType> => { //for all users to see
